...
WRAPPER="$HOME/.local/bin/mstd"
# 已有入口的保护：绝不用 > 直接写（symlink 会被穿透，毁掉它指向的真实文件）。
# 不是本脚本生成的东西一律先挪成 .bak，再用临时文件 + mv 原子替换。
if [ -L "$WRAPPER" ] || { [ -e "$WRAPPER" ] && ! grep -q '^# generated by mstd install.sh$' "$WRAPPER" 2>/dev/null; }; then
  mv "$WRAPPER" "$WRAPPER.bak"
  say "提示：$WRAPPER 已存在且不是本脚本生成的，已备份为 $WRAPPER.bak"
fi
{
  printf '#!/usr/bin/env bash\n'
  printf '# generated by mstd install.sh\n'

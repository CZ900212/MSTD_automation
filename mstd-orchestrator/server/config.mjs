...
// 数字旋钮统一走 intEnv fail-fast：裸 Number() 会让垃圾值静默变 NaN（且 NaN ?? default 仍是 NaN），
// 流进 debounce 定时器/预算判断后表现成怪行为而非启动报错。index.mjs 的进程级旋钮同样复用。
export function intEnv(env, key, fallback, { min = 1 } = {}) {
  if (!(key in env)) return fallback;
  const raw = String(env[key] ?? "");
  const parsed = Number(raw);
  if (!raw || !Number.isSafeInteger(parsed) || parsed < min) {
...
export function loadServerConfig(env = process.env) {
  ...
  return {
    ...
    dailyTokenBudget: intEnv(env, "MSTD_DAILY_TOKEN_BUDGET", 2_000_000),
    sessionTokenBudget: intEnv(env, "MSTD_SESSION_TOKEN_BUDGET", 300_000),
    ...
    dreamingMode: "shadow",
    debounceAddressedMs: intEnv(env, "MSTD_DEBOUNCE_ADDRESSED_MS", 600),
    debounceAmbientMs: intEnv(env, "MSTD_DEBOUNCE_AMBIENT_MS", 1500),
    debounceMaxMs: intEnv(env, "MSTD_DEBOUNCE_MAX_MS", 3000),
  };
}

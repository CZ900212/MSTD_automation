...
3. {"action":"escalate","brief":"一句话概括用户诉求","ack":"立即先发出的一句应答"} —— 需要认真处理:任何写操作意图(建任务/发消息/日程等)、**需要判断+解释的问题**(选型/建议/方案权衡/征求看法)、复杂问题、涉及第三人、正式对外内容、需要查资料,以及一切**复述/总结/回顾**类请求(完整上下文不在你手里,必须升级)。ack 必填:像同事先接住话,如"这个我捋一下哈,马上说"/"收到,我看看";一句话以内,绝不预告具体结论,不客服腔。
...
export function createTriage({ caller, store, soul = "", windowTokens = 2048 }) {
  async function triage({ session, items, mode, snapshot = null, brainBusy = false }) {
    ...
    const verdict = enforce(parse(out.text), items, mode);
  ...
  function parse(text) {
    try {
      ...
      if (j.action === "escalate" && typeof j.ack !== "string") delete j.ack;
      return j;
    } catch {
  ...
  function enforce(verdict, items, mode) {
    ...
    if (mode !== "ambient" && ["quick_reply", "no_reply"].includes(verdict.action) && RECAP_INTENT.test(joined)) {
      ...
      return { action: "escalate", brief, ack: DEFAULT_ACK };
    }
    ...
    if (ADVICE_INTENT.test(joined)) {
      return { action: "escalate", brief: `判断/建议类问题,需观点+理由分条回复：${joined.slice(0, 100)}`, ack: DEFAULT_ACK };
    }
    if ((verdict.text ?? "").length > QUICK_REPLY_MAX || WRITE_INTENT.test(verdict.text ?? "")) {
      return { action: "escalate", brief: `用户消息需正式处理：${items.map((i) => i.content).join("；").slice(0, 100)}`, ack: DEFAULT_ACK };
    }
    return verdict;
  }

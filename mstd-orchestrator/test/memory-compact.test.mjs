...
describe("上下文压缩 + flush + nudge", () => {
  ...
  describe("compact 流程", () => {
    ...
    it("flush 先于摘要；近 20 条原文保留；压缩点落 system 摘要", async () => {
      ...
      expect(brain.turn).toHaveBeenCalledWith(expect.objectContaining({
  ...
  // P0：tool-internal 不进入记忆压缩模型输入。
  it("earlyText 排除 tool-internal", async () => {
    ...
    expect(earlyText).not.toContain("内部X");
  });
